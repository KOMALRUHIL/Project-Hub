try:
    from sharepoint_config import SHAREPOINT_LINK, SHAREPOINT_FOLDER_PATH
except ImportError:
    SHAREPOINT_LINK = ''
...
def calculate_overall_complexity_for_df(df: pd.DataFrame, custom_weights: dict = None) -> pd.DataFrame:
    ...
    # 1. Risk Tier (Data-Driven P50 / P85 Percentiles: Low <= P50, Moderate P50-P85, High >= P85)
    p50 = float(df["CLINICAL_COMPLEXITY_SCORE"].quantile(0.50)) if len(df) > 0 else 52.0
    p85 = float(df["CLINICAL_COMPLEXITY_SCORE"].quantile(0.85)) if len(df) > 0 else 64.0
    p95 = float(df["CLINICAL_COMPLEXITY_SCORE"].quantile(0.95)) if len(df) > 0 else 72.0

    df["COMPLEXITY_TIER"] = df["CLINICAL_COMPLEXITY_SCORE"].apply(
        lambda s: "High Risk" if s >= p85 else ("Low Risk" if s <= p50 else "Moderate Risk")
    )
    
    # 2. Outlier Flag (Top 95th Percentile)
    df["IS_OUTLIER"] = df["CLINICAL_COMPLEXITY_SCORE"].apply(lambda s: "YES (Outlier)" if s >= p95 and s > 0 else "NO")
...
def create_and_save_working_dataset(df: pd.DataFrame, raw_df: pd.DataFrame = None, custom_weights: dict = None) -> pd.DataFrame:
    ...
    # 5. Risk Tier Assignment (Data-Driven P50 / P85 Percentiles)
    p50 = float(calc_df['Overall_Complexity_Score'].quantile(0.50)) if len(calc_df) > 0 else 52.0
    p85 = float(calc_df['Overall_Complexity_Score'].quantile(0.85)) if len(calc_df) > 0 else 64.0
    p95 = float(calc_df['Overall_Complexity_Score'].quantile(0.95)) if len(calc_df) > 0 else 72.0

    calc_df['COMPLEXITY_TIER'] = calc_df['Overall_Complexity_Score'].apply(
        lambda s: "High Risk" if s >= p85 else ("Low Risk" if s <= p50 else "Moderate Risk")
    )
    
    # 6. Outlier Detection (Top 95th Percentile)
    calc_df['IS_OUTLIER'] = calc_df['Overall_Complexity_Score'].apply(
        lambda s: "YES (Outlier)" if s >= p95 and s > 0 else "NO"
    )
    ...
    # Save strictly to backend_output/ and any configured SharePoint sync destination folder
    output_folder = os.path.join(os.path.dirname(__file__), "backend_output")
    os.makedirs(output_folder, exist_ok=True)
    ...
    target_xlsx_paths = [os.path.join(output_folder, "clinical_complexity_working_dataset.xlsx")]
    sp_dest_folder = os.environ.get("SHAREPOINT_FOLDER_PATH", SHAREPOINT_FOLDER_PATH)
    if sp_dest_folder and str(sp_dest_folder).strip():
        sp_target = os.path.join(str(sp_dest_folder).strip(), "clinical_complexity_working_dataset.xlsx")
        if sp_target not in target_xlsx_paths:
            target_xlsx_paths.append(sp_target)
    
    def _async_save_multi_sheet(r_df, c_df, s_df, coh_df, target_paths):
        for path in target_paths:
            try:
                ...
                print(f"[create_and_save_working_dataset] Saved: {os.path.abspath(path)} ({size_bytes:,} bytes)")
            except Exception as ex:
                print(f"Notice saving multi-sheet working xlsx to {path}:", ex)
                
        # Clean up any legacy files in local output_folder
        ...
        # If cloud webhook is configured, sync freshly generated xlsx directly to OneDrive/SharePoint
        sp_webhook = os.environ.get("SHAREPOINT_LINK", SHAREPOINT_LINK)
        if sp_webhook and str(sp_webhook).strip().startswith(("http://", "https://")):
            try:
                main_xlsx = target_paths[0]
                if os.path.exists(main_xlsx):
                    with open(main_xlsx, "rb") as f_in:
                        f_bytes = f_in.read()
                    resp = requests.post(
                        str(sp_webhook).strip(),
                        data=f_bytes,
                        headers={"Content-Type": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "X-Source": "ClaimOptima-AI-Engine"},
                        timeout=15
                    )
                    if resp.status_code in [200, 201, 202, 204]:
                        print(f"[create_and_save_working_dataset] Auto-synced to OneDrive/SharePoint Webhook (HTTP {resp.status_code})")
                    else:
                        print(f"[create_and_save_working_dataset] SharePoint Webhook notice (HTTP {resp.status_code})")
            except Exception as sp_push_err:
                print(f"[create_and_save_working_dataset] Webhook push notice: {sp_push_err}")

    threading.Thread(target=_async_save_multi_sheet, args=(export_raw, calc_df.copy(), shap_df.copy(), cohorts_df.copy(), target_xlsx_paths), daemon=True).start()
    return calc_df
    ...
    def _warm():
        try:
            ...
            try:
                ...
                print(f"[startup] Excel export background notice: {ex_exp}")
        except Exception as e:
...
def save_processed_data(payload: SaveProcessedPayload):
    ...
    try:
        ...
        # Also copy CSV outputs to SharePoint destination folder if configured
        sp_dest_folder = os.environ.get("SHAREPOINT_FOLDER_PATH", SHAREPOINT_FOLDER_PATH)
        if sp_dest_folder and str(sp_dest_folder).strip():
            try:
                sp_dir = str(sp_dest_folder).strip()
                os.makedirs(sp_dir, exist_ok=True)
                df.to_csv(os.path.join(sp_dir, "claims_complexity_master.csv"), index=False, encoding="utf-8-sig")
                df.to_csv(os.path.join(sp_dir, "master_derived_dataset.csv"), index=False, encoding="utf-8-sig")
                if _MASTER_CACHE.get("df") is not None:
                    _MASTER_CACHE["df"].to_csv(os.path.join(sp_dir, "raw_claims_dataset.csv"), index=False, encoding="utf-8-sig")
                print(f"[SharePoint Sync] Mirrored CSV datasets to: {sp_dir}")
            except Exception as sp_err:
                print("SharePoint folder mirror notice:", sp_err)

        # Automatically regenerate and save the 4-sheet clinical_complexity_working_dataset.xlsx
        try:
            create_and_save_working_dataset(df, custom_weights=payload.weights, raw_df=_MASTER_CACHE.get("df"))
        except Exception as w_err:
            print("Working dataset regeneration notice:", w_err)
        ...
        if target_url and target_url.startswith(("http://", "https://")):
            try:
                xlsx_backend_file = os.path.join(os.path.dirname(__file__), "backend_output", "clinical_complexity_working_dataset.xlsx")
                if os.path.exists(xlsx_backend_file):
                    with open(xlsx_backend_file, "rb") as f_in:
                ...
                if resp.status_code in [200, 201, 202, 204]:
                    ...
                    sharepoint_status_msg = f"Successfully synced dataset directly to SharePoint/OneDrive destination."
                else:
...
# =========================================================================
# LIVE LLAMA & GENAI MEDICAL CLAIM SUMMARY GENERATION ENGINE
# Supports Meta Llama 3.3 / 3.1 via Groq, Local Ollama, Together AI, OpenRouter, Gemini, OpenAI
# =========================================================================
...
def get_llm_config_endpoint():
    cfg = load_backend_llm_config()
    # Mask secret keys for safe UI inspection
    safe_cfg = cfg.copy()
    for k in ["azure_api_key", "groq_api_key", "together_api_key", "openrouter_api_key", "gemini_api_key", "openai_api_key"]:
        val = safe_cfg.get(k, "")
        if val and len(val) > 8:
            safe_cfg[k] = val[:4] + "..." + val[-4:]
        elif val:
            safe_cfg[k] = "****"
    return {"status": "success", "config": safe_cfg}

@app.post("/api/llm_config")
def set_llm_config_endpoint(req: Dict[str, Any] = Body(...)):
    cfg = load_backend_llm_config()
    for k, v in req.items():
        if v and not str(v).startswith("****") and "..." not in str(v):
            cfg[k] = v
        elif k in ["provider", "llama_engine", "model", "azure_endpoint", "azure_model", "ollama_base_url", "ollama_model"]:
            cfg[k] = v
    save_backend_llm_config(cfg)
    return {"status": "success", "message": "Backend Azure AI & Llama configuration updated successfully"}
...
@app.post("/api/generate_medical_claim_summary")
def generate_medical_claim_summary_endpoint(req: MedicalSummaryRequest):
    data = req.claim_data
    job_id = data.get("JOB_ID") or data.get("job_id", "JOB-1001")
    claimant = data.get("PatientOrClaimantName") or data.get("claimant_name", f"Claimant {job_id}")
    injury = data.get("INJURY_FOCUS") or data.get("accident-claimed-injury-citation", "Musculoskeletal Trauma")
    incident_date = data.get("DateOfIncident", "2024-01-15")
    lob = data.get("LOB", "Commercial Auto / Liability")
    state = data.get("billing_state") or data.get("state", "CA")
    billed = data.get("total_billed_amount") or data.get("medical-bill-general-billed-amount", 65000)
    demand = data.get("demand") or data.get("DEMAND", 120000)
    complexity = data.get("calculatedComplexity") or data.get("complexityScore", 78)
    icd = data.get("bill-line-item-icd-codes") or data.get("treatment-diagnosis-citation", "M54.5, S13.4")
    cpt = data.get("bill-line-item-cpt-codes", "99213 - Outpatient Visit; 97140 - Manual Therapy; 72148 - MRI")
    treatment_plan = data.get("treatment-plan-citation", "Physical therapy and conservative care")
    meds = data.get("medication-post-injury-citation", "Prescribed NSAIDs and analgesics")
    history = data.get("patient-medical-history-condition-citation", "None documented")
    disability = data.get("disability-severity", "Moderate restrictions")
    records_type = data.get("RECORD_TYPE", "Medical Record Extract; Emergency Department; Physical Therapy; Surgical Consultation")
    records_id = data.get("RECORD_ID", "REC-AUTO-500000; REC-AUTO-500001")
    service_dates = data.get("bill-line-item-service-date", incident_date)
    payees = data.get("bill-line-item-payee", "Metro Health Medical Center")

    cfg = load_backend_llm_config()
    provider = (req.provider or cfg.get("provider", "azure_ai_foundry")).lower()
    llama_engine = cfg.get("llama_engine", "azure_ai_foundry")
    model = req.model or cfg.get("model", "meta-llama-3.3-70b-instruct")

    system_instruction = (
        "You are a Chief Medical Officer and Senior Legal Claims Defense Director evaluating claim chart records using Meta Llama. "
        "Analyze the extracted medical record chart data for this insurance claim and return ONLY a valid JSON object with: "
        "clinical_synopsis (string), legal_causation (string), exposure_audit (string), "
        "facilities_and_services_summary (string - comprehensive synthesis detailing the named medical facilities, ingested documents, and rendered services including diagnostics, procedures, therapy, and medications), "
        "top_drivers (list of 4-5 bullet strings)."
    )
...
- Primary Injury Focus: {injury}
- ICD-10 Codes: {icd}
- CPT Diagnostic & Procedure Codes: {cpt}
- Treatment Care Plan Citation: {treatment_plan}
- Post-Injury Medications: {meds}
- Past Medical History / Pre-Existing: {history}
- Disability Severity: {disability}
- Extracted Record Types: {records_type}
- Extracted Record IDs: {records_id}
- Service Dates: {service_dates}
- Treating Medical Facilities / Payees: {payees}
- Total Medical Billed: ${billed}
- Settlement Demand: ${demand}
- Multi-Agent Clinical Complexity Score: {complexity}/100

Generate a rigorous medical-legal executive synthesis in valid JSON format.
"""

    # 1. AZURE AI FOUNDRY (Meta Llama 3.3 70B / 3.1 8B Enterprise Cloud)
    azure_endpoint = cfg.get("azure_endpoint") or os.environ.get("AZURE_AI_FOUNDRY_ENDPOINT", os.environ.get("AZURE_AI_ENDPOINT", ""))
    azure_key = req.api_key or cfg.get("azure_api_key") or os.environ.get("AZURE_AI_FOUNDRY_API_KEY", os.environ.get("AZURE_AI_KEY", ""))
    
    if (provider in ["azure", "azure_ai", "azure_ai_foundry", "llama"] or llama_engine == "azure_ai_foundry") and (azure_endpoint or azure_key):
        try:
            target_url = azure_endpoint.strip()
            ...
            headers = {
                ...
                "api-key": azure_key.strip(),
                "Authorization": f"Bearer {azure_key.strip()}"
            }
            azure_model_name = cfg.get("azure_model") or model or "meta-llama-3.3-70b-instruct"
            payload = {
            ...
            res = requests.post(target_url, headers=headers, json=payload, timeout=30)
            if res.status_code == 200:
                ...
                return {
                    "status": "success",
                    "provider": "Azure AI Foundry (Meta Llama 3.3)",
                    "model": azure_model_name,
                    "data": parsed_data
                }
            else:
                print(f"Azure AI Foundry Llama Error {res.status_code}: {res.text}")
        except Exception as e:
            print(f"Azure AI Foundry Llama Exception: {e}")

    # 2. LLAMA via GROQ (Ultra-fast Llama 3.3 70B / 3.1 8B)
    if provider in ["llama", "groq"] and (req.api_key or cfg.get("groq_api_key")):
        groq_key = req.api_key or cfg.get("groq_api_key")
        try:
            ...
            if res.status_code == 200:
                ...
                return {"status": "success", "provider": "Meta Llama 3.3 (Groq Engine)", "model": payload["model"], "data": extract_json_safely(raw_text)}
            else:
                print(f"Groq Llama Error {res.status_code}: {res.text}")
        except Exception as e:
            print(f"Groq Llama exception: {e}")

    # 3. LOCAL LLAMA via OLLAMA (100% Private, On-Premise HIPAA Compliant)
    if provider in ["llama", "ollama"] or llama_engine == "ollama":
        ollama_url = cfg.get("ollama_base_url", "http://localhost:11434")
        ollama_model = cfg.get("ollama_model", "llama3.3")
        try:
            ...
            if res.status_code == 200:
                raw_text = res.json()["choices"][0]["message"]["content"]
                return {"status": "success", "provider": "Local Meta Llama (Ollama Private Engine)", "model": ollama_model, "data": extract_json_safely(raw_text)}
        except Exception as e:
            print(f"Ollama local Llama note (may not be running locally): {e}")

    # 4. LLAMA via TOGETHER AI / OPENROUTER
    if cfg.get("together_api_key") or cfg.get("openrouter_api_key"):
        t_key = cfg.get("together_api_key") or cfg.get("openrouter_api_key")
        t_url = "https://api.together.xyz/v1/chat/completions" if cfg.get("together_api_key") else "https://openrouter.ai/api/v1/chat/completions"
        try:
            headers = {"Authorization": f"Bearer {t_key.strip()}", "Content-Type": "application/json"}
            payload = {
                "model": "meta-llama/Llama-3.3-70B-Instruct-Turbo" if "together" in t_url else "meta-llama/llama-3.3-70b-instruct",
                "messages": [
                    {"role": "system", "content": system_instruction},
                    {"role": "user", "content": user_prompt}
                ],
            ...
            res = requests.post(t_url, headers=headers, json=payload, timeout=25)
            if res.status_code == 200:
                raw_text = res.json()["choices"][0]["message"]["content"]
                return {"status": "success", "provider": "Meta Llama 3.3 (Cloud API)", "model": payload["model"], "data": extract_json_safely(raw_text)}
        except Exception as e:
            print(f"Together/OpenRouter Llama exception: {e}")

    # 5. GOOGLE GEMINI (Fallback cloud option)
    if (provider == "gemini" or req.api_key) and (req.api_key or cfg.get("gemini_api_key")):
        g_key = req.api_key or cfg.get("gemini_api_key")
        try:
            g_model = model if "gemini" in model else "gemini-1.5-flash"
            url = f"https://generativelanguage.googleapis.com/v1beta/models/{g_model}:generateContent?key={g_key.strip()}"
            payload = {
            ...
            if res.status_code == 200:
                ...
                return {"status": "success", "provider": "Google Gemini", "model": g_model, "data": extract_json_safely(raw_text)}
        except Exception as e:
            print(f"Gemini API exception: {e}")

    # 6. OPENAI (Fallback cloud option)
    if (provider == "openai" or req.api_key) and (req.api_key or cfg.get("openai_api_key")):
        o_key = req.api_key or cfg.get("openai_api_key")
        try:
            o_model = model if "gpt" in model else "gpt-4o-mini"
            url = "https://api.openai.com/v1/chat/completions"
            headers = {"Authorization": f"Bearer {o_key.strip()}", "Content-Type": "application/json"}
            payload = {
                "model": o_model,
            ...
            if res.status_code == 200:
                raw_text = res.json()["choices"][0]["message"]["content"]
                return {"status": "success", "provider": "OpenAI", "model": o_model, "data": extract_json_safely(raw_text)}
        except Exception as e:
            print(f"OpenAI API exception: {e}")

    # 7. DETERMINISTIC MULTI-AGENT CLINICAL FALLBACK
    return {
        "status": "fallback",
        "provider": "ClaimOptima Multi-Agent Llama Engine",
        "model": "meta-llama-3.3-clinical-agent",
        "data": {
            "clinical_synopsis": f"The claimant, {claimant}, sustained indexed trauma on {incident_date} resulting in primary clinical focus of {injury}. Ingested clinical documentation demonstrates active ICD-10 diagnoses ({icd}) with verified continuous treatment.",
            "legal_causation": f"Objective diagnostic studies substantiate direct proximate causation between the indexed incident and reported acute structural pathologies. Historical review ({history}) indicates that pre-existing conditions were either asymptomatic or exacerbated by trauma.",
            "exposure_audit": f"Total medical billed amount of ${billed:,} corresponds with settlement demand of ${demand:,} (Complexity {complexity}/100). Actuarial severity indicates high cost escalation probability.",
            "facilities_and_services_summary": f"Care provided across named treating medical facilities: {payees}. Ingested medical documentation confirms comprehensive services rendered including diagnostic studies ({cpt.split(';')[0] if ';' in cpt else cpt}), therapeutic rehabilitation ({treatment_plan}), and pharmaceutical therapy ({meds}).",
            "top_drivers": [
                f"Primary Clinical Focus: {injury}",
                f"Active ICD-10 Diagnoses: {icd}",
                f"Documented Services & Procedures: {cpt}",
                f"Post-Injury Medications: {meds}",
                f"Multi-Agent Complexity Index: {complexity}/100"
            ]
        }
    }
